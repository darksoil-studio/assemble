...
/**
 * @element collective-commitments-for-call
 */
@localized()
@customElement('collective-commitments-for-call')
export class CollectiveCommitmentsForCall extends LitElement {

  // REQUIRED. The CallHash for which the CollectiveCommitments should be fetched
  ...
  _collectiveCommitments = new StoreSubscriber(this, () =>
    this.assembleStore.collectiveCommitmentsForCall.get(this.callHash)
  );
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No collective commitments found for this call")}</span>
      </div>`;
