...
export class CreateSatisfaction extends LitElement {
  ...
  async createSatisfaction(fields: any) {
    ...
    try {
      ...
      const record: EntryRecord<Satisfaction> = await this.assembleStore.client.createSatisfaction(satisfaction);

      this.dispatchEvent(new CustomEvent('satisfaction-created', {
        composed: true,
        bubbles: true,
        detail: {
