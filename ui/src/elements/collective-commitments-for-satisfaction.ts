...
export class CollectiveCommitmentsForSatisfaction extends LitElement {

  // REQUIRED. The SatisfactionHash for which the CollectiveCommitments should be fetched
  ...
  _collectiveCommitments = new StoreSubscriber(this, () =>
    this.assembleStore.collectiveCommitmentsForSatisfaction.get(this.satisfactionHash)
  );
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No collective commitments found for this satisfaction")}</span>
      </div>`;
